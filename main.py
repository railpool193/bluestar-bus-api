...
# ------------------------- Live URL autodetekció (VÁLTOZATLAN) -------------------------
...
def _build_extra_headers() -> dict:
    ...
    if os.getenv("X_API_KEY"):
        h["X-API-Key"] = os.getenv("X-API_KEY")
    
    # Hozzáadjuk a kulcsot az URL-hez, és beállítjuk az Accept fejlécet XML-re
...
def _format_vm_url(line_ref: str):
    ...
    if not u:
        return "", {}
    # A korábban látott URL-hez hasonlóan, hozzáadjuk az explicit XML formátumot
...
# ------------------------- Beépített sablonok (VÁLTOZATLAN) -------------------------
...
# ------------------------- Live hívások (Javítva: XML támogatás) -------------------------
LIVE_CACHE = {}
...
async def http_get_xml(url, params=None) -> bytes:
    ...
    if not url or httpx is None:
        return b""
    try:
        # Összefűzzük a fő headereket a hívásspecifikus headerekkel
        all_headers = EXTRA_HEADERS.copy()
        if params and "LineRef" in params:
             # Ezt a DFT SIRI nem igényli, de a kódod eredeti logikáját megtartjuk
            del params["LineRef"] 
        if params and "MonitoringRef" in params:
            del params["MonitoringRef"]
            del params["MaximumStopVisits"]
...
def _parse_iso(dt_str: str):
    try:
        if not dt_str:
            return None
        # támogatunk Z-t és mikroszekundumot is
        ...
        try:
            ...
            for d in deliveries:
                for v in d.findall('siri:MonitoredStopVisit', SIRI_NAMESPACES):
                    ...
                    trip_id_str = trip_ref_e.find('siri:DatedVehicleJourneyRef', SIRI_NAMESPACES).text if trip_ref_e is not None else ""
    ...
    for st in stop_times_by_stop.get(sid, []):
        ...
        if live_hit:
            ...
            rows.append({
                ...
                "row_class": "timetable",
                "trip_id": tid,
                # A kódot itt vágtad el, a hiányzó rész feltételezhetően üres értékeket ad
                "fleet": "", 
                "delay_text": "",
            })
