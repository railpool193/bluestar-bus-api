...
def _lifespan(runtime: ApplicationRuntime):
    ...
    async def lifespan(_app):
        ...
        gtfs_started = live_started = False
        try:
            try:
...
def create_app(runtime: ApplicationRuntime | None = None) -> FastAPI:
    ...
    routers_and_endpoints = [
        ...
        create_status_router(runtime=StatusRuntime(runtime.gtfs_provider, runtime.live_provider, lambda: runtime.gtfs_refresh.snapshot(), runtime.now, settings.live_max_age_seconds, settings.live_operator_filter, settings.live_cache_ttl_seconds)),
        create_search_router(runtime=SearchRuntime(runtime.gtfs_provider), unavailable_response=api_error),
        create_stops_router(runtime=StopRuntime(runtime.gtfs_provider, runtime.live_provider, runtime.now, settings.departure_window_minutes, settings.departure_limit, settings.live_match_minutes), unavailable_response=api_error),
        create_trips_router(runtime=TripRuntime(runtime.gtfs_provider, runtime.live_provider, runtime.now), unavailable_response=api_error),
